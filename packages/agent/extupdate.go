...
// updateAllExtensions iterates $ZUT_HOME/extensions/* and tries to
...
func updateAllExtensions(zutHome string) {
	dir := filepath.Join(zutHome, "extensions")
	entries, err := os.ReadDir(dir)
	if err != nil {
		if os.IsNotExist(err) {
			return // no extensions installed; nothing to do
		}
		fmt.Fprintf(os.Stderr, "zut update: skipping extension update (read %s: %v)\n", dir, err)
		return
	}
	...
	fmt.Println()
	fmt.Println("zut update: updating extensions...")
	...
	fmt.Printf("zut update: extensions: %d updated, %d up-to-date, %d skipped, %d failed\n",
...
func gitStash(ctx context.Context, dir string) (stashRef string, stashed bool, err error) {
	...
	msg := fmt.Sprintf("zut-update-%d", time.Now().Unix())
	...
	if lerr == nil {
		for _, line := range strings.Split(listOut, "\n") {
			if strings.Contains(line, msg) {
				// line looks like: "stash@{0}: On main: zut-update-1700000000"
