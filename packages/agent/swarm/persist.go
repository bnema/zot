...
func (f *Swarm) sanitizeReloadMeta(stateDir, root string, m *agentMeta) error {
	if m == nil {
...
func validRuntimeInboxPath(path, id string) bool {
	...
	if !strings.HasPrefix(filepath.Base(dir), "zot-swarm-") {
		return false
	}
...
func (f *Swarm) Reload() (loaded int, errs []error) {
	roots := []string{f.cfg.Root}
	roots = append(roots, f.cfg.LegacyRoots...)
	...
	for _, root := range roots {
		...
		count, rootErrs := f.reloadRoot(root)
		loaded += count
...
func (f *Swarm) reloadRoot(root string) (loaded int, errs []error) {
...
func (f *Swarm) buildDetachedAgent(m agentMeta) *Agent {
	// Legacy meta.json files may not record a repository identity. Those
	// records are migrated to the current RepoRoot; current-format records
	// retain their persisted repository so a restart from another checkout
	// cannot silently redirect the child.
	dir := m.Dir
	if m.WorkspaceMode != WorkspaceWorktree && m.RepositoryRoot == "" && f.cfg.RepoRoot != "" {
		// Legacy metadata did not record the repository identity. Preserve
		// the historical best-effort migration only for those records.
		dir = f.cfg.RepoRoot
	}
	if dir == "" {
...
func (f *Swarm) ResumeSession(ctx context.Context, id string) (*Agent, error) {
	return f.resume(ctx, id, true)
}
...
func (f *Swarm) RestartTask(ctx context.Context, id string) (*Agent, error) {
	return f.resume(ctx, id, false)
}

func (f *Swarm) resume(ctx context.Context, id string, resuming bool) (*Agent, error) {
	f.operationMu.Lock()
	...
	case <-existing.done:
	default:
		return nil, fmt.Errorf("swarm: agent %s is still finalizing; wait before resuming", existing.ID)
	}
	...
	if inboxLive(existing.InboxPath) {
		return nil, fmt.Errorf("swarm: agent %s still has a live worker; send shutdown or stop it first", existing.ID)
	}
