...
func (f *Swarm) SpawnBatch(ctx context.Context, req BatchRequest) (*Batch, error) {
	if len(req.Tasks) == 0 {
...
func (f *Swarm) collectBatch(batch *Batch) {
...
func (f *Swarm) waitForBatchResult(a *Agent) (*TurnResult, error) {
	if a == nil {
...
func (f *Swarm) WaitBatch(id string) (*BatchResult, error) {
	f.mu.Lock()
	batch := f.batches[id]
	f.mu.Unlock()
	if batch == nil {
...
func (f *Swarm) GetBatch(id string) *Batch {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.batches[id]
}
...
func (f *Swarm) reloadBatches(root string) []error {
...
func (f *Swarm) persistBatch(batch *Batch) {
