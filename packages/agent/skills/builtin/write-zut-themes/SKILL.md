...
installing, debugging, or packaging a zut color theme. Read this skill
before generating a theme file or advising on theme extension layout.

## What a zut theme is

A zut theme is a JSON file that overrides any subset of zut's built-in
...
Valid; inherits all colors/spinner/syntax from zut defaults.
...
If `colors.light` is missing, zut applies `colors.dark` overrides on
...
```bash
mkdir -p "$ZUT_HOME/themes"
cp my-theme.json "$ZUT_HOME/themes/my-theme.json"
```

Then open `/settings` and choose **color theme**. zut switches theme
immediately and persists the selection in `$ZUT_HOME/config.json`.

If a selected theme file is deleted, zut resets the setting to the
built-in auto/default theme.

## Theme-only extensions

A zut extension can exist only to ship a theme. No slash command,
