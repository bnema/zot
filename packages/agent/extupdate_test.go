...
// makeFakeZutHome scaffolds $ZUT_HOME/extensions/ and returns the path.
func makeFakeZutHome(t *testing.T) string {
	t.Helper()
	home := t.TempDir()
...
func TestUpdateAllExtensions_NoExtensionsDirectory(t *testing.T) {
	// Brand new $ZUT_HOME with no extensions/ directory at all.
