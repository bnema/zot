#!/usr/bin/env bash
# Minimal shell client for the `zut rpc` JSON protocol.
#
# Usage:
...
# Pipe the command into zut rpc; pipe its stdout through jq to react
# to events. The trailing `cat` keeps the input pipe open until the
# subprocess exits on its own (after `done`).
{
  if [ -n "${ZUTCORE_RPC_TOKEN:-}" ]; then
    jq -nc --arg t "$ZUTCORE_RPC_TOKEN" '{id:"0",type:"hello",token:$t}'
  fi
  echo "$cmd"
  # Block here so stdin stays open until zut exits.
  cat
} | zut rpc | while IFS= read -r line; do
