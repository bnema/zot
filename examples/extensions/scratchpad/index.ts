// scratchpad — a real .ts zut extension with no SDK and no build step.
//
...
function log(msg: string): void {
  // stderr is captured by zut to $ZUT_HOME/logs/ext-<name>.log;
...
//
// Notes persist as JSONL under <cwd>/.zut/scratchpad-notes.jsonl so
// they survive zut restarts and stay scoped to the project. The path
...
//
// Single-writer assumption: only one zut session per cwd at a time.
// Concurrent writes from two zut instances would interleave but not
...
function setNotesPath(cwd: string): void {
  notesPath = join(cwd, ".zut", "scratchpad-notes.jsonl");
  loadNotes();
}
...
// Sentinel: tells zut all initial registrations are flushed so the
