# scratchpad — example zut extension (TypeScript, source-run)
...
This copies the manifest + source into `$ZUT_HOME/extensions/scratchpad/`.
...
Notes persist as JSONL at `<cwd>/.zut/scratchpad-notes.jsonl`. The
file is created on first `/note` and survives zut restarts. Each line
