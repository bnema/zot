...
All discovered files are appended to the prompt in that order, so a global baseline can be refined per project. To list their paths above the interactive transcript, enable **show loaded instructions at startup** in `/settings`. The setting is off by default, and the displayed path list is not sent to the model or stored in the session transcript.
...
- **show loaded instructions at startup**: list every loaded `AGENTS.md` file path above the transcript. Off by default. Changes apply immediately and persist to `config.json` as `show_instructions_at_startup`.
